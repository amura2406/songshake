...
- [ ] **[TEST]** `enrichment.py` tightly couples I/O — `process_playlist()` directly calls `storage`, `playlist`, `download_track`, `enrich_track`, and `genai.Client`. It does not use the `Protocol` classes defined in `platform/protocols.py`, making the core 200-line function untestable without real infrastructure. — [enrichment.py:166-378](file:///Users/anggar/Code/song-shake/src/song_shake/features/enrichment/enrichment.py#L166-L378)
